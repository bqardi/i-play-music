...
function PlayLists() {
	...
	function setPics(index) {
		...
		return {
			previous: (
				<CardImage
					onClick={() => setCurrIndex(prevIndex(index, length))}
					pointer
					src={prevItem.images[0].url}
					alt="Butterfly"
				/>
			),
			...
			next: (
				<CardImage
					onClick={() => setCurrIndex(nextIndex(index, length))}
					pointer
					src={nextItem.images[0].url}
	...
	return (
		<article className="PlayLists">
			...
			<div className="PlayLists__listen">
				<Link to={`/player/${content.items && content.items[currIndex].id}/all`} className="PlayLists__button">
					Listen All
				</Link>
			</div>
		</article>
