...
function Login({ validate }) {
	...
	var redirect_uri =
		process.env.NODE_ENV === "production"
			? "https://bqardi-iplaymusic.netlify.app/callback"
	...
	var action = `https://accounts.spotify.com/authorize?${queryParameters}`;

	return (
		<article className="Login">
			<h1 className="Login__title">Log In</h1>
			<form onSubmit={canLogin} action={action}>
				<LoginField field={usernameField} />
				<LoginField field={passwordField} />
				<button className="Login__button">Log In</button>
			</form>
			<div className="Login__onetouch">
				<button onClick={() => validate({ action })} type="submit" className="Login__fingerprint">
